...
const translations = {
  ne: {
    ...
    research: { eyebrow: 'चयनित अनुसन्धान', title: 'प्रमाणमा आधारित, <em>आगामी सम्भावनाप्रति</em> उन्मुख।', all: 'Google Scholar मा सबै हेर्नुहोस्', loading: 'प्रकाशनहरू लोड हुँदैछन्…', synced: 'ORCID अभिलेखसँग समक्रमित प्रकाशन सूची', fallback: 'सुरक्षित प्रकाशन सूची प्रदर्शन गरिँदैछ', read: 'प्रकाशन खोल्नुहोस्', originalTitle: 'मूल प्रकाशन शीर्षक', citations: 'उद्धरण' },
...
function renderPublications() {
  ...
  const allPublications = publicationData.publications || [];
  const publications = allPublications.slice(0, 8);
  const publicationCounter = document.querySelector('[data-publication-count]');
  if (publicationCounter) publicationCounter.dataset.count = String(allPublications.length);
...
function setupCounters() {
  ...
  const animate = element => {
    ...
    const tick = now => { const target = Number(element.dataset.count || 0); const progress = Math.min((now - start) / duration, 1); element.textContent = Math.floor(target * (1 - Math.pow(1 - progress, 3))); if (progress < 1) requestAnimationFrame(tick); };
