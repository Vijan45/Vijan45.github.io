...
A responsive, accessibility-aware portfolio for **Bhijan Neupane**, designed for GitHub Pages. It ships with an English-first interface, a Nepali toggle, an animated cyan–magenta–gold cosmic canvas, refined identity typography, verified publication links, profile links, and a managed publication-feed workflow.

## Included

- English is the default language; Nepali is available through the header language switch and the choice is retained locally. The v2 storage key intentionally resets the earlier Nepali-first preference for existing visitors.
...
- Site icon assets: refreshed BN cosmic-monogram SVG favicon, PNG favicons, Apple touch icon, PWA manifest, and social-preview image.
